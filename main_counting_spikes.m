...
% Loading filenames
  [filenames_train, class_train, filenames_test, class_test] = ...
    get_filenames_on_off_database(50);
  ...
  numel_files_to_train_features = 20;
...
specs_layer1.tau = 4000; % tau is an exeuclideanponential decay, in microseconds
...
all_train_context = aggregate_contexts_count(spikes_feature_train, specs_layer1, decay_rate);
t_compute_context = toc(t_start_compute_context);
fprintf('Contexts computed in %.1f seconds.\n', t_compute_context)

imagesc(all_train_context)
pause;
...
[~, centers.data] = kmeans(all_train_context, nb_clusters, 'Distance', 'sqeuclidean');
t_clustering = toc(t_start_clustering);
fprintf('Done in %.2f seconds.\n', t_clustering)

imagesc(centers.data)
pause
...
metric_dist_assignement = 'euclidean';
spikes_train = assign_closest_center_count(spikes_train, centers, metric_dist_assignement, decay_rate);
spikes_test = assign_closest_center_count(spikes_test, centers, metric_dist_assignement, decay_rate);
t_affectation = toc(tstart_affectation)
fprintf('Done in %.2f seconds.\n', t_affectation)


%% Keeping only closest events
thresh_dist_validity = 5;
...
spikes_feature_train = assign_closest_center_count(spikes_feature_train, centers, metric_dist_assignement, decay_rate);
...
all_train_context2 = aggregate_contexts_count(spikes_feature_train, specs_layer2, decay_rate);

thresh_too_far_event2 = 3*specs_layer2.tau; % in microseconds
ratio_empty_ctx2 = 0; % below this ratio the context will be discarded
nb_clusters2 = 20;
...
fprintf('Kmeans clustering...\n')
[~, centers2.data] = kmeans(all_train_context2, nb_clusters2, 'Distance', 'euclidean');
t_clustering2 = toc(t_start_clustering2);
fprintf('Done in %.2f seconds.\n', t_clustering2)

tstart_affectation2 = tic;
metric_dist_assignement = 'euclidean';
spikes_train2 = assign_closest_center_count(spikes_train, centers2, metric_dist_assignement, decay_rate);
cspikes_test2 = assign_closest_center_count(spikes_test, centers2, metric_dist_assignement, decay_rate);
...
make_sig_normalized = @(x) make_sig(x) / numel(find(x.validity));
sigs_train2 = cellfun(make_sig, spikes_train2, 'un', 0)';
sigs_train2 = vertcat(sigs_train2{:});
sigs_test2 = cellfun(make_sig, spikes_test2, 'un', 0)';
...
spikes_feature_train = assign_closest_center_count(spikes_feature_train, centers2, metric_dist_assignement, decay_rate);
for ind = 1:numel(spikes_feature_train)
  spikes_feature_train{ind}.validity = spikes_feature_train{ind}.dist_closest_center < thresh_dist_validity;
end
all_train_context3 = aggregate_contexts_count(spikes_feature_train, specs_layer3, decay_rate);

thresh_too_far_event3 = 3*specs_layer3.tau; % in microseconds
ratio_empty_ctx3 = 0; % below this ratio the context will be discarded
nb_clusters3 = 40;
...
fprintf('Kmeans clustering...\n')
[~, centers3.data] = kmeans(all_train_context3, nb_clusters3, 'Distance', 'euclidean');
t_clustering3 = toc(t_start_clustering3);
fprintf('Done in %.2f seconds.\n', t_clustering3)

tstart_affectation3 = tic;
metric_dist_assignement = 'euclidean';
spikes_train3 = assign_closest_center_count(spikes_train2, centers3, metric_dist_assignement, decay_rate);
spikes_test3 = assign_closest_center_count(spikes_test2, centers3, metric_dist_assignement, decay_rate);
